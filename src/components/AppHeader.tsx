...
import { ScrollText, Moon, Sun, Leaf, Settings, Sprout, Flame, ListTodo } from "lucide-react";
...
export const AppHeader = memo(function AppHeader({
    isDark, currentStreak,
    onToggleDark, onShowSettings, onShowTodo, onShowGarden, onShowHistory,
}: AppHeaderProps) {
    return (
            ...
            >
                ...
                {currentStreak > 0 && (
                    <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-foreground/5 text-foreground/60">
                        <Flame size={10} />
                        <span className="font-body text-[10px] font-medium">{currentStreak}</span>
                    </span>
                )}
            </motion.div>
            <motion.div
